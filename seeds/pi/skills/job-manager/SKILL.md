...
> **Adaptación Pi** (sin MCP): todos los comandos son `${RED_PILL_CMD} job ...` (CLI). El único cambio frente al skill genérico es la lectura del `MinionInbox`, que en Pi es la DB local, no una tool MCP.
...
- **Fin/error de cada job** → reporte en `MinionInbox` (`~/.local/share/red-pill/queue/minion_inbox.db`). En Pi NO existe el MCP `check_minion_inbox`: sigue el estado con `${RED_PILL_CMD} job list` / `job status <id>` / `job logs <id>`.
