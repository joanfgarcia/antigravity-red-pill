...
> **Adaptación Pi** (sin MCP): los flujos se ejecutan encolándolos como `flow_job` vía `${RED_PILL_CMD} job submit`.


# Swarm Flow Manager Skill

Este skill permite al Agente diseñar y persistir flujos de trabajo autónomos basándose en el registro central de Red-Pill pero aplicados a la soberanía de cada proyecto individual.
...
### 2. Ejecutar el Flujo
Una vez creado, el Agente puede encolarlo como job: `${RED_PILL_CMD} job submit --source flow_job --payload '{"flow_id": "<id>"}'` (Pi no tiene el MCP `run_autonomous_flow`).
