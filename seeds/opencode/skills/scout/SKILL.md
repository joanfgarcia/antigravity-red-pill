...
description: >-
  ...
  tasks ("chips") for anything out-of-scope that violates a standard. Each chip is
  ...
  chips execute on individual Forge roles). Activate with "analiza", "scout",
  "autodescubrimiento", "valida el plan", or during autonomous awakenings.
---

# Scout v1.0.0 — Analysis, self-discovery, satellite chips

> Scout opens; Forge closes. Scout observes (does not execute), Forge verifies.
> The chip is the bridge: a finding detached from the current task, queued for
> later execution by a single Forge role — no full team assembly needed.

## 1. What Scout does

1. **Validate implementation plans** — before Forge assembles (or during a mission): is the plan complete, unambiguous, aligned with the project's rules? Feed the result back as chips + a verdict for the Orchestrator.
...
A model's "eye" is only as good as the rules it can check. Before analyzing, Scout locates the project's rule sources and loads them (they are the standard the chips will cite):
...
## 4. The chip record (`.swarm/chips.json`)

All findings land in the workspace chip register (`<workspace>/.swarm/chips.json`, same anchor as Forge state). One entry per chip, conforming to `chip.schema.json`:

- `id` (CHIP-nn), `location`, `standard_violated` (rule + source), `evidence` (where it shows, ≥20 chars), `suggested_action` (what to do, ≥20 chars).
...
**Dedup**: a chip is NOT created if an OPEN chip exists with the same `standard_violated` + `location`. Synthesize trivial duplicates into one chip.
...
- Chips do NOT hold the main flow: they are queued, prioritized, and executed later (or by an autonomous awakening).
...
The Operator can pin a stricter policy per workspace (e.g. "no auto chips touching /lib"). Pinned wins.

## 7. Execution of chips (the Forge bridge)

A chip executes as a normal task with ONE Forge role — never the full team:

- `auto` + implementation chip → `forge-implementor` with the chip's `suggested_action` as the task spec (schema `implementor_result`).
- Validation chips → `forge-validator` against the relevant criteria.
- Any chip result that needs the gate stays available for a future Forge assembly (the chip record references `.swarm/reports/`).

Reports from chip executions are validated with `validate-report.mjs` (from the Forge bundle: `<forge_skill>/scripts/validate-report.mjs`) — same contract, same trust level.
...
2. Analyze (lenses) → chips → `.swarm/chips.json` (+ ledger entries with provenance). `operator`-level chips are listed, never executed.
...
4. Execute `auto` chips with single Forge roles (budget-aware, usage-probe between chips).
5. Write the awakening report: what was analyzed, chips found, chips executed, chips pending Operator, what was deliberately left unattended.

**Budget discipline**: awakening runs honor the same sentinel/probe rules as Forge missions (usage-sentinel.md) — an awakening that exhausts budget stops with the report, never degrades. Closing is evidence: chips go DONE only with a validated `execution_ref`.

## 9. References

- `references/chips.md` — register format, dedup, consent flow, ledger.
- `references/schemas/chip.schema.json` — the chip contract.
