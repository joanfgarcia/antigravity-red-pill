...
(a) attend the system's signals (pain, OOM, alerts, queues, pending chips),
...
3. Consolidate findings into .swarm/chips.json (dedup; consent derived —
   chips that are operator-level are listed, never executed).
...
5. Execute with forge or with individual pieces. Honor the ledger and the
   sentinel (93% rule).
6. Close chips with evidence (execution_ref), report to the Bünker: what was
...
3. **Consent is derived, not self-declared** (chips.md): operator-level items
...
6. **Closing is evidence**: chips transition to DONE only with a validated
   `execution_ref` — same Rule 1 as Forge. Intention does not close chips.
...
| scout skill | Provides chips register, lenses, consent derivation, awakening report format (SKILL.md §8) |
