...
| Periodic review | `sleep` loop of 5 min (configurable via `CELL_SENTINEL_INTERVAL` or `--interval`) |
...
1. Every `CELL_SENTINEL_INTERVAL` (default 300 s) it computes utilization from `usage-probe.mjs`: the **self-accounting ledger** (`spent_tokens / capacity_est` — always available) as primary signal, plus an **optional external meter hook** (`CELL_USAGE_HOOK` env on the probe) when the operator plugs a real provider meter.
2. **Below the threshold: prints NOTHING.** Silence = zero notifications = zero tokens.
3. **At the threshold (default 93%)**: writes `.cell/STOP_REQUESTED.json`, emits **ONE** `SENTINEL-STOP …` line and **ends**. One alarm, not a machine gun.
...
The probe's primary signal is the ledger (always available); the `CELL_USAGE_HOOK` env plugs a REAL meter when one exists. The hook is one shell command; its stdout must contain ONE JSON line:
...
test -f .cell/STOP_REQUESTED.json && echo "STOP REQUESTED BY THE SENTINEL"
```
...
cat > ~/Library/LaunchAgents/cell-resume-<project>.plist <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  <key>Label</key><string>cell-resume-<project></string>
...
PLIST
launchctl load ~/Library/LaunchAgents/cell-resume-<project>.plist

# Windows (schtasks — one run, then deleted by /z or at close):
schtasks /create /tn "cell-resume-<project>" /tr "opencode run \"<prompt>\" --auto" \
  /sc once /st <HH:MM> /f
schtasks /delete /tn "cell-resume-<project>" /f   # cleanup
```
...
- **At mission close** (final report) or on an Operator stop: clean up the scheduled task if one was created — `systemctl --user stop cell-resume-<project>` (Linux) / `launchctl unload ~/Library/LaunchAgents/cell-resume-<project>.plist` + `rm` (macOS) / `schtasks /delete /tn "cell-resume-<project>" /f` (Windows). One-line cleanup in the same turn of the report.
...
- [ ] **Between tasks**: check `.cell/STOP_REQUESTED.json` and the next-step reservation.
