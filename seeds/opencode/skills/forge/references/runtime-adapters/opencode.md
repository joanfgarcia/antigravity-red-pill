...
> **Opt-in to orchestration:** when the Operator activates this skill ("levantar equipo", "modo cell", "misión completa", "equipo de trabajo"), that invocation **constitutes the explicit opt-in** to multi-agent orchestration with the `task` tool that the skill instructs.
...
2. **Emit the schema-conforming JSON to `.cell/reports/<role>-<phase>.json`** and mention the file path in the result. The orchestrator validates with `validate-report.mjs` BEFORE trusting (advisory verdict).
...
3. **The input enters via prompt/args (real JSON, not string).** The output persists the main loop into `.cell/state.json` — role agents write only `.cell/reports/`.
...
2. Initialize .cell/state.json + execution-tracker (render)
