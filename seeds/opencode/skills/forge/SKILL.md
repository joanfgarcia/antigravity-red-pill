...
description: >-
  ...
  of the scout skill (analysis + satellite chips). Activate when the operator
  says "levantar equipo", "swarm mode", "team up", "full mission", "ejecuta el
...
1. **Context anchoring** — The Orchestrator IS the main loop of the conversation, NEVER a subagent. Its anchor persists on disk (`.swarm/state.json`).
...
- Operator says: "levantar equipo", "modo swarm", "equipo de trabajo", "tarea compleja".
...
Every protocol step is a feature. The orchestrator resolves each feature at assembly time into `.swarm/state.json` (`features{}` + `feature_rationale[]`), and re-evaluates at every escalation trigger.
...
**Cross-cutting (v1.2.0)**: scout (skill) composes the same pieces — chips from a scout analysis execute on individual roles without assembling the full team. Every piece emits provenance v3.1; the orchestrator stamps it at consolidation.

**Source of truth: `.swarm/state.json`.** Human renders (`execution-tracker.md`, `assumption_registry.md`, `coverage_matrix.md`, `implementation-report.md`, `qa-report.md`, `MISSION_REPORT.md`) are regenerated by `scripts/render-artifacts.mjs`. Never edit them by hand.
...
1. Orchestrator packs the task (and the anchor plan if one exists) into a `task` call to `forge-triage` → `triage_plan` JSON at `.swarm/reports/triage.json`, validated with `validate-report.mjs` against `triage_plan.schema.json`.
2. **`recommendation: "NOT_NEEDED"`** → the swarm protocol does NOT engage. The task runs plain (L0 inline at most). Nothing else to do — this is the gate doing its job.
...
4. Initialize `.swarm/state.json` and render the initial tracker.
...
Rules: copy-and-adapt, never run blind. Role prompts must pack ALL context (cold context inherits nothing) and instruct emitting the schema-conforming JSON to `.swarm/reports/<role>-<phase>.json`; the orchestrator validates with `validate-report.mjs` before trusting.
...
```bash
node <skill>/scripts/gate-check.mjs .swarm/state.json
```
...
```bash
node <skill>/scripts/render-artifacts.mjs .swarm/state.json .swarm
```
