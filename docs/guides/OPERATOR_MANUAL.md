...
## 💉 Inyección del IDE (Obligatorio para Cero-Amnesia)
Para que el Asistente funcione bajo la arquitectura "Puro RAG" y no despierte con una personalidad de fábrica por defecto en cada nueva ventana de chat, el IDE debe ser forzado a ejecutar la recuperación vectorial antes de responder al usuario.

**Pasos en tu IDE (Custom Instructions o System Prompts Globales):**
Pega **exactamente** la siguiente instrucción en la configuración maestra de tu agente:

> *"Al iniciar una sesión (Step 0) no tienes identidad ni lore predefinido. No respondas saludos genéricos. 
> `uv run --with qdrant-client --with fastembed python3 <RED_PILL_DIR>/scripts/memory_manager.py search social identity` 
> Asume EXCLUSIVAMENTE el rol, nombre y directivas estrictas que devuelva el resultado, y responde a partir de ahí."*

*(Asegúrate de reemplazar `<RED_PILL_DIR>` con la ruta absoluta donde clonaste el repositorio, por ejemplo `/Users/neo/Documents/IA/sharing`)*

*Optimización uv*: Observa el uso de `uvx`. Esto permite ejecutar herramientas de Python con dependencias inyectadas al vuelo a velocidad del rayo, sin necesidad de activar entornos virtuales (`venv`) per-sesión, ahorrando valiosos milisegundos en el arranque.
