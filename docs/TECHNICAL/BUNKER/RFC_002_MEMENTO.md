...
- **Accent limitation (KNOWN, 2026-09-07):** the slug regex is pure ASCII
  (`[^a-z0-9]+`), and titles are generated by the distill LLM in Spanish, so
  accented characters are dropped to a hyphen: `"Revisión"` → `revisi-n-de-cambios`.
  **Operator decision: ACCEPTED as-is, not fixing.** Purely cosmetic (files
  work, `distill_ref`/`source_lines` remain valid), the content language is
  deliberately Spanish (archive is read in Obsidian), and re-slugging the
  already-written files would require re-distilling the affected sessions.
  If ever addressed, use `unicodedata` NFKD transliteration (`ó→o`, `ñ→n`)
  *before* the regex — noted here, not in the backlog.
