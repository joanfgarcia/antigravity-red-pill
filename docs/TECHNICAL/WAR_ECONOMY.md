...
**Location**: [samantha_on_demand.py](../../src/red_pill/inference/samantha_on_demand.py) + [samantha_worker.py](../../src/red_pill/inference/samantha_worker.py) + [run_dual_bind.py](../../scripts/setup_background_model.sh) *(generated dual-bind daemon)*
...
**Location**: `run_dual_bind.py` (generated by [setup_background_model.sh](../../scripts/setup_background_model.sh), launched by `redpill-llm.service`) + [model_registry.py](../../src/red_pill/core/model_registry.py)

A single FastAPI daemon (the Sovereign Inference Proxy) that owns every local model. It loads the model **in-process** (`llama_cpp.Llama`) and binds two channels at once — hence *dual-bind*:

```
Client A ─┐
Client B ──┤── run_dual_bind (TCP 8760 + UDS) ── in-process model (GPU, or CPU worker)
Client C ──┘
```

| Function | Description |
|----------|-------------|
| **Selector routing** | Resolves `(task, model, thinking, custom, experimental)` under lock via `model_runtime` |
| **On-demand boot** | Loads the model on the first `/v1/chat/completions`; hot-swaps without restarting the service |
| **Priority-aware reaper** | Auto-unloads idle models: interactive 5 min, low-priority 10 s; `POST /unload` releases VRAM on demand |
| **Device cascade** | GPU → CPU-worker fallback (`device_fallback`, default `["gpu","cpu"]`) resolved at request time from free VRAM |

**Economy**: If the daemon is running (because the user is working in the IDE), Samantha on-demand reuses it instead of starting a new process. If it's not running, Samantha starts its own ephemeral process and shuts it down when finished. The former `hypervisor_daemon.py` (ephemeral `llama-server` proxy) was never wired to a service and was **purged** (2026-09-21); this dual-bind daemon is its successor.
...
**Ubicación**: [samantha_on_demand.py](../../src/red_pill/inference/samantha_on_demand.py) + [samantha_worker.py](../../src/red_pill/inference/samantha_worker.py) + [run_dual_bind.py](../../scripts/setup_background_model.sh) *(daemon dual-bind generado)*
...
**Ubicación**: `run_dual_bind.py` (generado por [setup_background_model.sh](../../scripts/setup_background_model.sh), lanzado por `redpill-llm.service`) + [model_registry.py](../../src/red_pill/core/model_registry.py)

Un solo daemon FastAPI (el Sovereign Inference Proxy) que posee todos los modelos locales. Carga el modelo **in-process** (`llama_cpp.Llama`) y enlaza dos canales a la vez — de ahí *dual-bind*:

```
Client A ─┐
Client B ──┤── run_dual_bind (TCP 8760 + UDS) ── modelo in-process (GPU, o worker CPU)
Client C ──┘
```

| Función | Descripción |
|---------|-------------|
| **Selector routing** | Resuelve `(task, model, thinking, custom, experimental)` bajo lock vía `model_runtime` |
| **Boot on-demand** | Carga el modelo en el primer `/v1/chat/completions`; hot-swap sin reiniciar el servicio |
| **Reaper con prioridad** | Auto-descarga modelos idle: interactivo 5 min, baja prioridad 10 s; `POST /unload` libera VRAM a voluntad |
| **Cascada de dispositivos** | Fallback GPU → worker CPU (`device_fallback`, default `["gpu","cpu"]`) resuelto en cada request según VRAM libre |

**Economía**: Si el daemon está levantado (porque el usuario está trabajando con la IDE), Samantha on-demand lo reutiliza en vez de arrancar un proceso nuevo. Si no está levantado, Samantha arranca su propio proceso efímero y lo apaga al terminar. El antiguo `hypervisor_daemon.py` (proxy efímero de `llama-server`) nunca se cableó a un servicio y fue **purgado** (2026-09-21); este daemon dual-bind es su sucesor.
