...
During idle time, the system invokes the profile carrying the `distillation` capability (default `qwen35_9b` — Qwen3.5-9B, multilingual, large context; the legacy Samantha-Mistral-7B is retired from this role) via ProviderRegistry to read the fast buffer. It does not just match keywords: it understands the interaction and distills the raw text into a strict JSON essence (classifying it as `work` or `social`).
