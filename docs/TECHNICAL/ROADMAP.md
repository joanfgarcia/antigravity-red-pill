...
- [ ] **Operator Lifecycle CLI (`bunker init`, `install`, `update`)**: Implement an automated hardware profiling (`bunker init`) that generates a declarative YAML profile, followed by a deterministic installer and updater (`bunker install/update`) that automates dependency syncing, db migrations, model downloading, Quadlet deployment, and systemd daemon reloading.
