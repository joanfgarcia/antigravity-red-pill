...
- [ ] **Operator Onboarding & One-Click Install (`bunker init` & `bunker install`)**: Implement an automated hardware profiling (`bunker init`) that generates a declarative YAML profile, followed by a deterministic installer (`bunker install`) that automates dependency syncing, model downloading, Quadlet deployment, and systemd daemon binding.
