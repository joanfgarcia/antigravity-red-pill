...
- [ ] **Sovereign Cryptographic Vault (Secrets Manager)**: Implement a centralized `pure-mls` secret manager to protect third-party tokens (GitHub, GitLab, ClickUp) without exposing them in plain-text `.env`. Single-member MLS group encrypts/decrypts a JSON dictionary of secrets atomically. *Naming note (2026-08-26): "Sovereign Vault" clashes with the historical docstrings of `utils/vault*.py` and with the original name of RFC-002 (now **Memento**); pick this feature's definitive name when implementation starts.*
