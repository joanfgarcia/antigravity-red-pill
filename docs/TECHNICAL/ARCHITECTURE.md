...
- **[NEW v6.8.6] Agentic Self-Assembly**: Decoupled the directory hierarchy into `WORKSPACE_ROOT` (Agentic environment) and `APP_ROOT` (Red-Pill implementation). This protects local source-code adaptations and provides an extensible boundary for auxiliary modules (e.g., `USER_ATLAS_DIR`, `AGENT_CORE_DIR`), supporting both the Developer profile and the end User profile seamlessly. For a visual representation, see the [Sovereign Directory Atlas](SOVEREIGN_ATLAS.md).
...
- **[NEW v7.22.1] Local Inference Governance (RFC-HARNESS-002 v3 / AD-030)**: the LLM daemon (`run_dual_bind.py`) evolves from "server of one fixed model" to the **local inference governance service**. A selector (`experimental > custom > model > task > config > env`, no aliases) switches models **in hot** under lock per request; `task_profiles.yaml` (curated tasks × candidate models) and `model_profiles.yaml` (hardware calibration + model behavior: `chat_format`/null native template, `tool_format`, `thinking`) are separated; `model_runtime.py` is the shared resolution source (merge by specificity: candidate > task > profile-behavior > profile-calibration > config > env) used by daemon AND clients. **Native reasoning (Granite 4.2)**: llama-cpp-python lacks `chat_template_kwargs`, so the daemon registers per-mode chat handlers (`granite-thinking`/`nothink`/`low`) from the GGUF native template, selected per request. Custom/experimental modes (virtual full profile), CPU→GPU fallup with tier re-evaluation after unload, `MINION_DEFAULT_PROFILE` rename, `llm:` recipe injection (`RP_LLM_*`), and `/v1/tokenize` (real token counts). The inventory RFC (RFC-HARNESS-001) is superseded/archived; `run_battle` moves to FASE 2. Details: `docs/TECHNICAL/DECISION_LOG.md` AD-030, `docs/GUIDES/RUNBOOK.md` §8760, `docs/GUIDES/HARDWARE_MODELS_BE_WATER.md`.
