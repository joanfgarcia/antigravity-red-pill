...
*   **Hardware Constraint (Chunking)**: Because consumer GPUs and neural networks suffer from context-window degradation, interactions exceeding 8k tokens must be split into chunks ($C_1$, $C_2$, $C_n$). This is a hardware necessity, not a biological one.
*   **Affective Culling**: The daemon assesses each chunk and assigns an `emotion` label and an `intensity` score (0.0 to 1.0). If a chunk contains only generic code formatting or standard API errors, it returns `emotion: neutral` and `intensity: < 0.3`. Our engine actively **culls** these nodes, deleting them entirely to preserve narrative purity and optimize vector space. Only chunks with true cognitive or emotional weight survive the filter.
