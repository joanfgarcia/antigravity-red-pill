...
| `COGNITIVE_ROUTER_ENABLED` | `True` | 05 | Signals Operator state transitions (compact `OPERATOR_COLOR` tag; meaning lives in the CHROMA KEY legend). |
| `TONE_ADAPTER_ENABLED` | `True` | 06 | Signals the tone chroma on transitions (compact tag; meaning lives in the CHROMA KEY legend). |
