...
def test_all_docs_reachable_from_index():
	...
	all_docs = {p.resolve() for p in DOCS_DIR.rglob("*.md") if p.name not in EXCLUDED}
	...
	if orphans:
		...
		msg = f"Found {len(orphan_list)} orphan doc(s) not reachable from docs/README.md:\n" + "\n".join(f"  - {o}" for o in orphan_list)
...
def test_no_broken_links_in_index():
	...
	if broken:
		msg = f"Found {len(broken)} broken link(s) in docs/README.md:\n" + "\n".join(f"  - {b}" for b in broken)
		raise AssertionError(msg)
