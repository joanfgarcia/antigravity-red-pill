...
from red_pill.utils.affect import AFFECT_MAP, get_affect_coordinates, get_emotional_stability_multiplier
...
class TestGetAffectCoordinates:
	...
	def test_average_of_two_known_emotions(self):
		...
		assert v == pytest.approx(exp_v, abs=1e-09)
		assert a == pytest.approx(exp_a, abs=1e-09)
...
class TestGetEmotionalStabilityMultiplier:
	...
	@pytest.mark.parametrize("intensity_low,intensity_high,emotion", [(1.0, 5.0, "fear"), (1.0, 10.0, "anger"), (1.0, 8.0, "joy")])
	def test_higher_intensity_yields_lower_or_equal_multiplier(self, intensity_low, intensity_high, emotion):
		...
		assert result_high <= result_low + 1e-09, f"{emotion}: higher intensity should be more stable. low={result_low:.4f}, high={result_high:.4f}"
	...
	def test_negative_valence_more_stable_than_positive_at_same_arousal(self):
		...
		assert result_neg <= result_pos + 1e-09, (
	...
	def test_partial_unknown_ignored_gracefully(self):
		...
		assert result_known == pytest.approx(result_mixed, abs=1e-09)
