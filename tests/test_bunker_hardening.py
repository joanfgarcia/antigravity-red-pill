...
class TestBunkerHardening(unittest.TestCase):
	def setUp(self):
		...
		self.manager.client.recreate_collection(collection_name="social_memories", vectors_config=VectorParams(size=384, distance=Distance.COSINE))
		...
		self.manager.client.upsert(
			...
			points=[
				PointStruct(
					id=ID_OPERATOR_MOOD,
					vector=[0.1] * 384,
					payload={"global": {"gray": 1.0}, "last_3d": {"red": 0.2, "gray": 0.8}, "last_7d": {"gray": 1.0}, "interaction_count": 10},
				)
			],
		)
	...
	def test_ingestion_gate_noise(self):
		...
		result = self.manager.add_memory(collection="social_memories", text=garbage_text, importance=1.0)
