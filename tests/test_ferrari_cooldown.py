...
def test_cooldown_decay_logic():
	...
	work_kws = ["implementa", "arregla", "trabaja", "ejecuta", "despliega", "commit"]

	# Turn 1: Explicit work keyword triggers work mode (casual active = False)
	_cr_state.register_turn("implementa el router", casual_kws, work_kws)
	...
	_cr_state.register_turn("qué opinas de esto", casual_kws, work_kws)
	assert not _cr_state.is_casual_active()
	assert _cr_state._consecutive_non_work_turns == 1

	# Turn 3: Second consecutive non-work prompt -> decays to casual mode
	_cr_state.register_turn("sí, suena interesante", casual_kws, work_kws)
...
def test_explicit_casual_keywords():
	...
	_cr_state.register_turn("vamos a charlemos un rato", casual_kws, work_kws)
	assert _cr_state.is_casual_active()
...
def test_work_keywords_precedence():
	...
	_cr_state.register_turn("arregla el bug ahora", casual_kws, work_kws)
	assert not _cr_state.is_casual_active()
	assert _cr_state._consecutive_non_work_turns == 0
...
async def test_cognitive_router_plugin_transitions():
	...
	cfg_mock = MagicMock(
		CASUAL_OVERRIDE_KEYWORDS=["relax", "charlemos"],
		WORK_MODE_KEYWORDS=["trabaja", "ejecuta", "despliega", "commit"],
		COGNITIVE_ROUTER_ENABLED=True,
	)
...
async def test_tone_adapter_plugin_transitions():
	...
	with (
		...
		_cr_state.register_turn("ejecuta los tests", ["relax", "charlemos"], ["trabaja", "ejecuta", "despliega", "commit"])
		res2 = await p.execute("ejecuta los tests")
		assert res2 == ""

		# Turn 3: 1st non-work turn -> no transition, returns empty
		_cr_state.register_turn("hola", ["relax", "charlemos"], ["trabaja", "ejecuta", "despliega", "commit"])
		res3 = await p.execute("hola")
		assert res3 == ""

		# Turn 4: 2nd non-work turn -> decays to casual, returns empty
		_cr_state.register_turn("qué tal todo", ["relax", "charlemos"], ["trabaja", "ejecuta", "despliega", "commit"])
		res4 = await p.execute("qué tal todo")
		assert res4 == ""
		assert _cr_state.is_casual_active()
