...
class TestWorkspaceMemorySync:
	...
	def test_preservation_hook_isolated_execution(self, temp_workspace):
		...
		hook_file.write_text("def get_preservation_context(root):\n\treturn f'ARCH CONTEXT FOR {root}'\n", encoding="utf-8")
		...
		hook_file2.write_text("def get_preservation_context(root):\n\treturn f'DIFFERENT CONTEXT FOR {root}'\n", encoding="utf-8")
	...
	def test_sync_projects_memories_chronologically(self, temp_workspace, mock_registry_file):
		...
		mock_point3.payload = {"content": "Other Project Decision", "created_at": 1500.0, "workspace": "other_project"}
	...
	def test_compaction_atomic_replace_and_backup(self, mock_resolve_path, mock_create_bridge, temp_workspace, mock_registry_file):
		...
		mock_bridge.prompt.return_value = ConversationResult(conversation_id="test", response="Compacted MEMORY.md content")
		...
		mock_bridge.prompt.return_value = ConversationResult(conversation_id="test2", response="", error="LLM failed")
		compact_workspace_memory(ws, mock_mm)
...
class TestMCPMemoryActions:
	...
	async def test_mcp_read_write_list_workspace_memory(self, mock_find_workspace, temp_workspace):
		...
		from red_pill.mcp_server import handle_list_workspace_memory, handle_read_workspace_memory, handle_write_workspace_memory
