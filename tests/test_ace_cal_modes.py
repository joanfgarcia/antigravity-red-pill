...
class TestPioneerMode:
	def test_pioneer_selects_pioneer_base(self):
		...
		for emotion, (v, a) in _PIONEER_MODEL.items():
			...
			assert amap[emotion] == pytest.approx((v, a), abs=1e-06), f"Mismatch for {emotion}"
	...
	def test_pioneer_neutral_origin(self):
		...
		amap = _build("PIONEER")
		assert amap["neutral"] == pytest.approx((0.0, 0.0), abs=1e-09)
...
class TestCustomOverrides:
	def test_custom_overrides_single_emotion_on_pioneer(self):
		...
		assert amap["joy"] == pytest.approx((0.99, 0.01), abs=1e-09)
	...
	def test_custom_overrides_single_emotion_on_academic(self):
		...
		assert amap["fear"] == pytest.approx((-1.0, 1.0), abs=1e-09)
		assert amap["joy"] == pytest.approx(_ACADEMIC_MODEL["joy"], abs=1e-06)
	...
	def test_custom_overrides_new_emotion(self):
		...
		amap = _build("PIONEER", overrides)
		assert "awe" in amap
		assert amap["awe"] == pytest.approx((0.6, 0.85), abs=1e-09)
	...
	def test_custom_overrides_multiple_emotions(self):
		...
		amap = _build("PIONEER", overrides)
		assert amap["joy"] == pytest.approx((0.5, 0.5), abs=1e-09)
		assert amap["fear"] == pytest.approx((-0.5, 0.5), abs=1e-09)
		assert amap["neutral"] == pytest.approx((0.1, 0.1), abs=1e-09)

	def test_invalid_json_does_not_crash(self):
		"""Malformed CUSTOM_OVERRIDES JSON is silently ignored — PIONEER base returned."""
		amap = _build("PIONEER", "{not valid json}")
		assert amap["joy"] == pytest.approx(_PIONEER_MODEL["joy"], abs=1e-06)
	...
	def test_empty_overrides_returns_base(self):
		...
		for emotion, coords in _PIONEER_MODEL.items():
			assert pioneer[emotion] == pytest.approx(coords, abs=1e-09)
	...
	def test_unknown_model_falls_back_to_pioneer(self):
		...
		for emotion, coords in _PIONEER_MODEL.items():
			assert amap[emotion] == pytest.approx(coords, abs=1e-06)
