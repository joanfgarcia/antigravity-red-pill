...
def _discover_linux() -> list[dict]:
	...
	try:
		...
		result = subprocess.run(["pgrep", "-f", "language_server"], capture_output=True, text=True)
...
def _discover_macos() -> list[dict]:
	...
	servers = []
	try:
		result = subprocess.run(["pgrep", "-f", "language_server_macos"], capture_output=True, text=True)
		for pid in result.stdout.strip().split("\n"):
			if not pid.strip():
				continue
...
def _find_ports_linux(pid: int) -> list[int]:
	...
	# Try netstat
	try:
		result = subprocess.run(["netstat", "-lnpt"], capture_output=True, text=True, timeout=5)
	...
	# Try ss
	try:
		result = subprocess.run(["ss", "-lnpt"], capture_output=True, text=True, timeout=5)
		for line in result.stdout.split("\n"):
...
def _find_ports_macos(pid: int) -> list[int]:
	...
	ports = []
	try:
		result = subprocess.run(["lsof", "-p", str(pid), "-i", "-P", "-n"], capture_output=True, text=True)
		for line in result.stdout.split("\n"):
			if "LISTEN" in line:
...
def find_all_endpoints(
	...
	from red_pill.utils.antigravity_history.api import call_api
