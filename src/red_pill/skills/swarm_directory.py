    ...
    def execute(self, community_alias: str):
        ...
        try:
            ...
            if not members:
                 return {
                     "status": "success",
                     "message": f"La comunidad '{community_alias}' está vacía actualmente."
                 }
                 
            # Format output
            member_list = [f"- {data['alias']} (Estado: {data['status']})" for key, data in members.items() if data.get('community') == community_alias]
            ...
            return {
                "status": "error", 
                "message": f"Error al intentar consultar el directorio base de datos Firebase Admin: {e}"
            }
