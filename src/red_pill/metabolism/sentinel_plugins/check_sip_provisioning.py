...
class SipProvisioningCheck(ServiceSentinelPlugin):
	...
	def _audit_provisioning(self, cfg: Any) -> List[AuditFinding]:
		...
		if not dual_bind.exists():
			...
			corrupt = self._check_dual_bind_content(dual_bind)
			if corrupt:
	...
	def _check_dual_bind_content(self, dual_bind: Path) -> Optional[AuditFinding]:
		"""Validate the generated daemon is syntactically sane and defines the manager.

		run_dual_bind.py is GENERATED by setup_background_model.sh; a partial deploy
		can leave it present but broken — e.g. missing `ModelManager`, which raises
		NameError on every /v1/chat/completions (HTTP 500) and silently black-holes
		sleep consolidation and the Memento pass. Existence alone is not enough.
		"""
		try:
			source = dual_bind.read_text(encoding="utf-8")
		except Exception as e:
			return AuditFinding(
				type="sip_corrupt_dual_bind",
				severity=8.0,
				message=f"{self.name}: run_dual_bind.py at {dual_bind} is not readable: {e}.",
				metadata={"service": self.service_unit, "path": str(dual_bind)},
			)

		try:
			compile(source, str(dual_bind), "exec")
		...
		if not re.search(r"^\s*manager\s*=\s*ModelManager\(\)", source, re.MULTILINE):
			return AuditFinding(
				type="sip_corrupt_dual_bind",
				severity=8.0,
				message=(
					f"{self.name}: run_dual_bind.py at {dual_bind} is missing "
					"`manager = ModelManager()` — the service will 500 on every request. "
					"Regenerate via setup_background_model.sh."
				),
				metadata={"service": self.service_unit, "path": str(dual_bind)},
			)
		return None
	...
	def heal_specific(self, cfg: Any, finding: AuditFinding) -> bool:
		...
		try:
			...
			try:
				...
				mm.evaporate_signals("signal_sip_corrupt_dual_bind_failure")
