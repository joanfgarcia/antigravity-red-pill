...
class NotaryOffice:
	...
	def propose_knowledge(self, content: str, vector: List[float], metadata: Dict[str, Any]) -> bool:
		...
		engram = {"content": content, "vector": vector, "metadata": metadata}
	...
	def audit_and_sign(self, proposal: Dict[str, Any]) -> bool:
		...
		if not proposal_id:
			logger.warning("Audit failed: missing proposal_id.")
			return False
		...
		return self.transport.notarize_proposal(str(proposal_id), self.agent_id, signature)
