...
async def test_vestido():
	...
	payload_rag = {"retrieved_engrams": [{"id": "doc-A", "weight": 1.0, "last_accessed": viejo_ts}]}

	mutated = await plugin.hook(PluginScope.MEMORY, payload_rag)
	peso_resultante = mutated["retrieved_engrams"][0]["weight"]

	print(" * Engrama extraído tras 10 días dormido.")
	print(" * Peso original: 1.000")
	...
	payload_cognitivo = {"active_engram_id": "axon-1234", "operator_friction": True}

	_ = await plugin.hook(PluginScope.COGNITION, payload_cognitivo)
