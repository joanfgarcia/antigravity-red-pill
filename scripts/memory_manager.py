...
if __name__ == "__main__":
	...
	elif collection_type == "core":
		collection = "core_directives"
	else:
	...
	if cmd == "add":
		...
		for i, hit in enumerate(results):
			print(f"[{i}] UUID: {hit.id} | Score: {hit.score:.4f} | R: {hit.payload.get('reinforcement_score', 0):.2f}")
			print(f"Content: {hit.payload.get('content')}\n")
	elif cmd == "delete":
		# collection_type here acts as the collection name, content acts as point_id
		# but usually we want to specify: delete social_memories <id>
		...
		try:
			...
			# UUID string
			pid = point_id
		client.delete(collection_name=coll_name, points_selector=[pid])
		print(f"Directive/Memory deleted: {pid} from {coll_name}")
