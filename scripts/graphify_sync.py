...
def _emit_failure(detail: str) -> None:
	...
	try:
		...
		MemoryManager().inject_signal("knowledge_graph_stale", intensity=6.0, signal_type="pain", source="GRAPHIFY_SYNC")
...
def _git_head(proj: Path) -> str | None:
	try:
		r = subprocess.run(["git", "-C", str(proj), "rev-parse", "HEAD"], capture_output=True, text=True, check=False)
...
def sync(dry_run: bool = True, only: str | None = None) -> int:
	...
	for w in targets:
		...
		listed = {p["path"]: bool(p.get("enabled", False)) for p in (manifest.get("projects") or []) if isinstance(p, dict) and p.get("path")}
		...
		for path, enabled in listed.items():
			...
			if dry_run:
				...
				cmd = ["systemd-run", "--user", "--scope", "-p", f"MemoryMax={MEM_MAX}", "graphify", "update", str(proj), "--no-cluster"]
