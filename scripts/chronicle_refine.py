...
def main():
	...
	while True:
		response = mem.client.scroll(
			...
			scroll_filter=models.Filter(must_not=[models.FieldCondition(key="type", match=models.MatchValue(value="idea_fragment"))]),
			limit=50,
			with_payload=True,
			with_vectors=False,
			offset=offset,
		...
		for p in points:
			...
			for f_idx, frag in enumerate(fragments):
				...
				mem.add_memory(collection=collection, text=frag["content"], point_id=f_node_id, metadata=f_payload, importance=3.0)
