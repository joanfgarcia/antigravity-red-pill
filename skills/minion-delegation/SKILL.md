...
description: Use when you are about to delegate/offload a single task to a minion — spawn a sub-agent, run_agent_task, pick a backend (claude/agy/opencode/local/local-tools/pi), assign work to another model, or run a bounded local tool loop. Picks the right minion KIND for the task and shows how to invoke it via swarm_orchestrator_api. For composing multi-step minion FLOWS use swarm-flow-manager instead.
---
