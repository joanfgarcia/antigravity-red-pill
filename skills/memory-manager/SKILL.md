...
`${RED_PILL_CMD} search social identity`
...
- **Search**:
  `${RED_PILL_CMD} search <collection> <text_to_search>`
  *Valid collections*: `social` (bond, identity, pacts) or `work` (code, technical decisions).

- **Memorize (Add)**:
  `${RED_PILL_CMD} add <collection> "<text>"`

- **Delete (Purge)**:
  `${RED_PILL_CMD} delete <collection> <uuid>`

- **Edit (v5.4.0)**:
  `${RED_PILL_CMD} edit <collection> <uuid> --color <color> --emotion <emotion> --intensity <float>`
