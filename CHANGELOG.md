...
- **[BENCH] Distiller bake-off (`scripts/distiller_bakeoff.py` + `docs/BENCHMARKS/DISTILLER_BAKEOFF.md`)**: aptitude harness (technical/philosophical/noise/emotional probes, deterministic scoring) run on GPU across the candidates. Outcome overturned the a-priori pick: **hermes_8b** is the production distiller (4/4 strict JSON, Spanish, no `<think>`, ~1 s), `piaget_8b` the affective-depth alternative; Qwen3.5-9B rarely closes valid JSON (verbose reasoning) and `samantha` (0/4) is retired. Harness supports GPU / CPU / hybrid partial-offload. See [DISTILLER_SELECTION.md](docs/TECHNICAL/COGNITIVE/DISTILLER_SELECTION.md).
